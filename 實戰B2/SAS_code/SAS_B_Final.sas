...
%winsor(dsetin=complete9, dsetout=complete10, byvar = yyyy, vars = risk_premium , type=delete , pctl=5 95); /*trim at 5% and 95%*/
...
proc means data=SampleU n mean std median min p1 p5 p10 p25 p50 p75 p90 p95 p99 max;
	var risk_premium Lag_she Lag_mshe Lag_netg Lag_idiorisk;
run;
...
proc means data=SampleZ n mean std median min p1 p5 p10 p25 p50 p75 p90 p95 p99 max;
	var risk_premium Lag_shr Lag_mshe Lag_netg Lag_idiorisk;
	by yyyy;
run;
...
proc reg data=Estimate1 outest=Coeff     ; 	
	model risk_premium= Lag_shr Lag_mshe Lag_netg Lag_idiorisk ;
quit;
...
data PredictA3;
	set PredictA2;
	Predict_risk_premium=B0+B1*Lag_shr+B2*Lag_mshe+B3*Lag_netg+B4*Lag_idiorisk;
