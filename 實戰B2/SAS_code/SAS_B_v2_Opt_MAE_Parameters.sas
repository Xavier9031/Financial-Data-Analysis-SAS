...
%winsor(dsetin=complete9, dsetout=complete10, byvar = yyyy, vars = risk_premium , type=delete , pctl=5 95); /*trim at 3% and 97%*/
...
proc means data=SampleU n mean std median min p1 p5 p10 p25 p50 p75 p90 p95 p99 max;
	var risk_premium Lag_shr Lag_mshe;
run;
...
**Step1:以2005~2019資料進行訓練與估計係數，自變數lag 1期;
data Estimate1;
	set SampleZ;
	where yyyy between 2005 and 2019;
run;
...
proc reg data=Estimate1 outest=Coeff; 	
	model risk_premium= Lag_shr Lag_mshe;
quit;
...
data PredictA3;
	set PredictA2;
	Predict_risk_premium=B0+B1*Lag_shr+B2*Lag_mshe;
