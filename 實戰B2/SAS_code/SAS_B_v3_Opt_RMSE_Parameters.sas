...
proc means data=SampleU n mean std median min p1 p5 p10 p25 p50 p75 p90 p95 p99 max;
	var risk_premium Lag_shr Lag_Tobins_Q_A;
run;
...
**Step1:以2018~2019資料進行訓練與估計係數，自變數lag 1期;
data Estimate1;
	set SampleZ;
...
proc reg data=Estimate1 outest=Coeff; 	
	model risk_premium= Lag_shr Lag_Tobins_Q_A;
quit;
...
data PredictA3;
	set PredictA2;
	Predict_risk_premium=B0+B1*Lag_shr+B2*Lag_Tobins_Q_A;
