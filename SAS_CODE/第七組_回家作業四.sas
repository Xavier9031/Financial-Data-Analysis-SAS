...
data Ex5_4; 
	set Mydata. F01s_c_ucgi_owner_summary;
	yyyy = year(DataDate);
	if NDirector<=10 then GroupDirector="6~10";
	else if NDirector<=15 then GroupDirector="11~15";
	else GroupDirector="16~20";
run;
proc freq data=	Ex5_4 ;
	table yyyy*GroupDirector  ;
run;
